...
} from '../settlement/hubMilestones.js';
import {
...
import {
  ...
  getDebriefContentAlpha,
  getDebriefHeaderColors,
  getDebriefScrollSafeArea,
  formatDebriefAssaultHeader,
  clipDebriefParchment,
  debriefLineFits,
  planDebriefBodyLayout,
  drawParchmentInk,
...
let gridPanX      = 0;
let gridPanY      = 0;
let _assaultWorldPadX = 0;
let _assaultWorldPadY = 0;
let _assaultWorldW    = COLS * CELL_SIZE;
let _assaultWorldH    = ROWS * CELL_SIZE;
let isPanning         = false;
...
let _hallImmersiveActive = false;       // true when immersive hall base was drawn this frame
let _treasuryFocusKey = null;           // selected fortress node in Treasury view
let _treasuryImmersiveActive = false;   // true when immersive treasury base was drawn this frame
let _barracksImmersiveActive = false;   // true when immersive barracks base was drawn this frame
let _battleMinimapRect = null;          // hit area for assault minimap (set each draw)
...
let _promotionQueueInitial = 0;  // total promotions when queue first shown (for "1 / N")
let _promoBannerTimer    = 0;   // auto-dismiss countdown (600 frames = 10s at 60fps)
let _chronicleUnread     = false; // amber dot on OPEN CHRONICLE until first open
...
// Map selection
let gamePhase        = 'slotSelect';  // slotSelect | settlementHub | campaignSelect | nodeMap | mapSelect | playing | debrief | betweenBattles
let _hubBtns           = [];
let _fromSettlementHub = false;
let _hubPulseBuilding  = null;
let _progressionBuilding = null; // warband | recruit | fortress | runeSmith — hub interior mode
...
function initBattle(preset) {
  ...
  const wantAshfen = _campaignNodeMode && isFirstSagaMap(_campaignMapIndex);
  if (wantAshfen) {
    initTerrain({
      ashfen: true,
      pathless: !!preset.pathless,
      assaultWorld: !!preset.pathless,
      spawn: preset.spawn,
      goal: preset.goal,
    });
  } else if (terrainUsesSprite) {
    initTerrain({ ashfen: false });
  }
...
function recordBattleResult(result, { skipDebrief = false } = {}) {
  ...
  _chronicleUnread = true;
  if (!_campaignState.uiHints) _campaignState.uiHints = {};
  _campaignState.uiHints.chronicleUnread = true;
  ...
  for (const t of _xpTowerPool) {
    ...
    if (newScars.length > 0) {
      ...
      for (const scarId of newScars) {
        ...
        if (scarId === 'bond_grief') sfxBondGrief();
      }
    }
  }
  ...
  _promotionQueueInitial = _promotionQueue.length;
...
function useAssaultScrollWorld() {
  return assaultPlayfieldWide() && _campaignNodeMode && isPathlessMode();
}

function getAssaultSpawnPixel(spawnCol = SPAWN.col, spawnRow = SPAWN.row) {
  if (!useAssaultScrollWorld()) {
    return { x: spawnCol * CELL_SIZE + CELL_SIZE / 2, y: spawnRow * CELL_SIZE + CELL_SIZE / 2 };
  }
  return getAssaultBorderSpawnPx(
    { col: spawnCol, row: spawnRow },
    GOAL,
    COLS,
    ROWS,
    CELL_SIZE,
...
function getAssaultEnemyPath(spawnCol = SPAWN.col, spawnRow = SPAWN.row) {
  if (!useAssaultScrollWorld()) {
    return currentPath?.map(({ col, row }) => grid.cellCenter(col, row)) ?? [];
  }
  return getAssaultBorderSpawnPath(
    { col: spawnCol, row: spawnRow },
    GOAL,
    COLS,
    ROWS,
...
/** Pathless assault — hide cell grid during active combat waves only. */
function hideAssaultBattleGrid() {
  if (isFortressPrepPhase()) return true;
  if (isCampaignAssaultBattle() && isPathlessMode()) {
    return waveState === 'active';
  }
  return isPathlessMode() && gamePhase === 'playing' && waveState === 'active';
}

/** Shared immersive rect for Hall / Treasury / Barracks (base + overlay must match). */
function getImmersiveWarCampBounds(W, H) {
  const _hallView = shouldShowHallOfHeroesView(_warCampTab, _progressionBuilding);
  const _treasuryView = shouldShowFortressUpgradeView(_warCampTab, _progressionBuilding);
  const _barracksView = isCampaignWarCamp()
    && shouldShowBarracksRecruitView(_warCampTab, _progressionBuilding);
  const _immersiveView = _hallView || _treasuryView || _barracksView;
  const _wcLayout = isCampaignWarCamp();
  const contentTop = _wcLayout
    ? (_immersiveView && _progressionBuilding
      ? FRAME_THICK + META_TOP_BAR_COMPACT_H + 4
      : getWarCampContentTop(FRAME_THICK))
    : META_SCREEN_TOP;
  const contentBot = H - FRAME_THICK - 8 - (_wcLayout && !_immersiveView ? WAR_CAMP_BOTTOM_BAR_H : 0);
  return { contentTop, contentBot };
}
...
function combatRightPanelX() {
  if (assaultPlayfieldWide() || commanderPrepLayout()) {
    return BASE_W - FRAME_PAD - combatRightPanelW();
  }
  return GRID_LEFT + COLS * CELL_SIZE + 4;
}
...
function playfieldShiftX() {
  if (useAssaultScrollWorld()) return 0;
...
function serializeGameSession() {
  ...
  if (gamePhase === 'settlementHub' && _hubPulseBuilding) {
    session.hubPulseBuilding = _hubPulseBuilding;
  }
...
function restoreGameSession(session) {
  ...
  switch (session.gamePhase) {
    ...
    case 'settlementHub':
      enterSettlementHub();
      if (session.hubPulseBuilding) _hubPulseBuilding = session.hubPulseBuilding;
      break;
    case 'nodeMap':
      gamePhase = 'nodeMap';
      gameOver = false;
...
function activateSlot(slotIndex) {
  ...
  _chronicleUnread = !!_campaignState.uiHints?.chronicleUnread;
  ensureCampaignProgress();
  loadRosterFromCampaignState();
  battlesCompleted = _campaignState.battlesCompleted ?? 0;
  stars = _campaignState.stars ?? 0;
...
function heroTouchesFortification(tower) {
  for (const [dc, dr] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
    const c = tower.col + dc;
    const r = tower.row + dr;
    const cell = grid.getCell(c, r);
    if (cell === CELL.WALL || cell === CELL.GATE) return true;
  }
  return false;
}

/** Y-sort walls then defenders — south wall segments must not cover hero sprites. */
function drawFortificationsAndTowersDepthSorted(ctx, time) {
  for (let row = 0; row < ROWS; row++) {
    for (let col = 0; col < COLS; col++) {
      const type = grid.getCell(col, row);
      if (type === CELL.WALL || type === CELL.GATE) {
        grid.drawFortificationAt(ctx, col, row, time);
      }
    }
  }
  ...
  const sorted = [...towers].sort((a, b) => {
    ...
    const aHero = isHeroTowerType(a.type) ? 1 : 0;
    const bHero = isHeroTowerType(b.type) ? 1 : 0;
    return aHero - bHero;
  });
...
/** Second sprite pass — heroes beside/over walls draw full body above fortifications. */
function redrawHeroSpritesOverFortifications(ctx, time) {
  for (const t of towers) {
    if (!isHeroTowerType(t.type)) continue;
    if (!isPathlessMode() && !heroTouchesFortification(t)) continue;
    t.drawCombatSpriteOnly(ctx, time);
  }
}
...
function drawHeroCombatHpBar(tower) {
  ...
  if (tower._rankIndex != null && tower._rankIndex < VETERAN_RANKS.length - 1) {
    ...
    ctx.beginPath();
    ctx.arc(bx - 4, by + barH / 2, 3, 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.restore();
}
...
function initHeroCombatHp(tower) {
  tower.combatHp    = 100;
  tower.combatMaxHp = 100;
  const def = _roster?.find(tower.defenderId);
  if (def) {
    const rank = getRank(def);
    tower._rankIndex = VETERAN_RANKS.findIndex(r => r.id === rank.id);
  }
}
...
function startCampaignNodeBattle(mapIndex, nodeIndex, options = {}) {
  ...
  if (isFirstSagaMap(mapIndex) && isFirstSagaAssaultNode(nodeIndex)) {
    const gateBonus = getFirstSagaGateHpBonus(nodeIndex);
    if (gateBonus > 0) {
      for (const w of Object.values(wallData)) {
        if (w.isGate) {
          const base = w.maxHp ?? GATE_HP;
          w.maxHp = base + gateBonus;
          w.hp = w.maxHp;
        }
      }
    }
...
function enterSettlementHub() {
  _fromSettlementHub = false;
  _progressionBuilding = null;
  _campaignRegionActive = true;
  if (_campaignMapIndex == null) _campaignMapIndex = 0;
  gameOver = false;
  victory = false;
  _returnToNodeMapAfterDebrief = false;
...
function openProgressionBuilding(opts = {}) {
  _fromSettlementHub = true;
  _progressionBuilding = opts.tab ?? hubBuildingToProgressionMode(opts.hubId) ?? 'warband';
  _hallFocusDefenderId = null;
  _treasuryFocusKey = null;
  enterCampaignWarCamp({
    freshVisit: opts.freshVisit ?? true,
    openRecruit: opts.openRecruit,
    singleBuilding: true,
    warCampTab: opts.tab,
  });
  if (opts.expandRuneCarver) _runeCarverExpanded = true;
}

function handleHubBuildingAction(action) {
  const hubState = {
  ...
  switch (action) {
    case 'openCommandMap':
      ...
      break;
    case 'openWarband': {
      const avail = getHubBuildingMilestone('warband', hubState);
      if (!avail.available) {
        _uiToast = { text: avail.reason ?? 'Locked.', timer: 90, color: UI_COLORS.gold };
        break;
      }
      _hubPulseBuilding = null;
    ...
    case 'openFortress': {
      const avail = getHubBuildingMilestone('fortress', hubState);
      if (!avail.available) {
        _uiToast = { text: avail.reason ?? 'Locked.', timer: 90, color: UI_COLORS.gold };
        break;
      }
      _hubPulseBuilding = null;
      openProgressionBuilding({ tab: 'fortress' });
      break;
    }
    case 'openRecruit': {
      const avail = getHubBuildingMilestone('recruit', hubState);
      if (!avail.available) {
        _uiToast = { text: avail.reason ?? 'Locked.', timer: 90, color: UI_COLORS.gold };
        break;
      }
      _hubPulseBuilding = null;
      openProgressionBuilding({ tab: 'recruit', openRecruit: true });
      break;
    }
    case 'openChronicle': {
      const avail = getHubBuildingMilestone('chronicle', hubState);
      if (!avail.available) {
        _uiToast = { text: avail.reason ?? 'Locked.', timer: 90, color: UI_COLORS.gold };
        break;
      }
    ...
    case 'openRuneSmith': {
      const avail = getHubBuildingMilestone('runeSmith', hubState);
      if (!avail.available) {
        _uiToast = { text: avail.reason ?? 'Locked.', timer: 90, color: UI_COLORS.gold };
        break;
      }
      _hubPulseBuilding = null;
      openProgressionBuilding({ tab: 'runeSmith', expandRuneCarver: true });
      break;
    }
    case 'openSkirmish': {
      const avail = getHubBuildingMilestone('skirmish', hubState);
      if (!avail.available) {
        _uiToast = { text: avail.reason ?? 'Locked.', timer: 90, color: UI_COLORS.gold };
        break;
      }
...
function enterCampaignWarCamp(opts = {}) {
  ...
  if (opts.freshVisit) _battleResult = null;
  if (!opts.singleBuilding) _progressionBuilding = null;
  _campaignRegionActive = true;
  _returnToNodeMapAfterDebrief = false;
  gameOver = false;
  victory = false;
  if (opts.warCampTab) {
    _warCampTab = opts.warCampTab === 'runeSmith' ? 'warband' : opts.warCampTab;
    _betweenSubtab = _warCampTab;
  } else {
    _warCampTab = opts.singleBuilding && _progressionBuilding
      ? (_progressionBuilding === 'runeSmith' ? 'warband' : _progressionBuilding)
      : 'warband';
    _betweenSubtab = _warCampTab;
  }
  _betweenFadeIn = 30;
  ...
  if (battlesCompleted === 1 && !_hintSeen.renameHint) {
    _renameHintUntil = performance.now() + 4000;
    _hintSeen.renameHint = true;
    if (_campaignState) {
      _campaignState.uiHints = { ...(_campaignState.uiHints ?? {}), renameHint: true };
      try { persistCampaign(); } catch {}
    }
  }
...
function switchCommandMapRegion(mapIndex) {
  const progress = ensureCampaignProgress();
  if (mapIndex == null || mapIndex < 0 || mapIndex >= progress.mapsUnlocked) return;
  if (isFirstSagaSliceLockedRegion(mapIndex)) return;
  _campaignMapIndex = mapIndex;
  _campaignMapPage = Math.floor(mapIndex / CAMPAIGN_MAPS_PER_PAGE);
...
function routeAfterHeroNaming() {
  ...
  } else if (pending.action === 'retryAssault' || pending.action === 'replayAssault') {
    enterFieldPrep(_campaignMapIndex, pending.nodeIndex ?? _campaignNodeIndex);
  } else if (pending.action === 'commandMap') {
    _commandMapView = _lastClearedFrontId ? 'front' : 'overview';
    _selectedFrontId = _lastClearedFrontId;
    gamePhase = 'nodeMap';
  } else if (pending.action === 'warCamp' || pending.action === 'settlementHub') {
    enterSettlementHub();
  } else {
    enterSettlementHub();
  }
...
function routeAfterCampaignDebrief(btn) {
  ...
  } else if (btn.action === 'warCamp' || btn.action === 'settlementHub') {
    enterSettlementHub();
  } else if (btn.action === 'retryAssault' || btn.action === 'replayAssault') {
    enterFieldPrep(_campaignMapIndex, btn.nodeIndex ?? _campaignNodeIndex);
...
function canRecruitInCampaignWarCamp() {
  const count = _roster?.defenders?.length ?? 0;
  const barracksLevel = _campaignState?.fortressUpgrades?.barracks ?? 0;
  const cap = getBarracksDisplayCap({
    firstSagaMap: isCampaignWarCamp() && isFirstSagaMap(_campaignMapIndex),
    barracksLevel,
  });
  if (isCampaignWarCamp() && isFirstSagaMap(_campaignMapIndex)) {
  ...
  if (count >= cap) {
    const needLv = Math.min(3, Math.max(0, count - 2));
    return {
      ok: false,
      reason: count >= MAX_FIELD_HEROES
        ? `Warband full — ${MAX_FIELD_HEROES} heroes maximum.`
        : `Roster full — upgrade Barracks to Lv.${needLv + 1}.`,
    };
  }
  return { ok: true };
}
...
function enterFieldPrep(mapIndex, nodeIndex = null) {
  ...
  const rawField = run?.fieldState ?? { gold: 0, towers: [], walls: {} };
  const field = repairFieldStateAfterAssault(rawField, rawField.deploySnapshot ?? null);
...
function processPrepShellClick(action) {
  ...
  if (action.action === 'warCamp' || action.action === 'settlementHub') {
    persistCampaignFieldLayout();
    enterSettlementHub();
    return;
  }
...
function processEnemyMeleeAttacks() {
  ...
  for (const enemy of enemies) {
    ...
    let dmg = enemy.isBoss ? 28 : Math.max(8, Math.round(12 + enemy.maxHp * 0.02));
    if (_campaignNodeMode && isFirstSagaMap(_campaignMapIndex)) {
      const meleeScale = getFirstSagaMeleeDamageScale(_campaignNodeIndex);
      if (meleeScale !== 1) dmg = Math.max(4, Math.round(dmg * meleeScale));
    }
...
let particles     = [];
let screenShake   = 0;
let combatHitStop = 0;   // micro freeze frames on heavy hits
let goldSpent     = 0;
let goldEarned    = 0;
...
function startNextWave() {
  ...
  sfxWaveStart();
  if (_campaignNodeMode && isFirstSagaMap(_campaignMapIndex) && waveNumber === 1) {
    const _sagaTitles = {
      0: 'FIRST NIGHT — THE MIST STIRS',
      1: 'ASH ROAD — RAIDERS IN THE LANE',
      2: 'BROKEN WATCH — HOLD THE RING',
      3: 'MENDED WOOD — THE FEN CLOSES IN',
    };
    const title = _sagaTitles[_campaignNodeIndex] ?? 'SAGA ASSAULT';
    chapterBannerText  = title;
    chapterBannerTimer = Math.max(chapterBannerTimer, 240);
    if (_campaignNodeIndex === 0) sfxChapterBanner();
  }
...
function updateWave() {
  ...
  if (spawnQueue.length > 0) {
    ...
    if (spawnTimer >= spawnInterval) {
      spawnTimer = Math.floor(Math.random() * Math.max(1, Math.floor(baseSpawnGap * 0.35)));
...
function initTerrain(options = {}) {
  const assaultWorld = options.assaultWorld === true;
  const ws = assaultWorld ? assaultWorldSize(COLS, ROWS, CELL_SIZE) : null;
  const W  = assaultWorld ? ws.width : COLS * CELL_SIZE;
  const H  = assaultWorld ? ws.height : ROWS * CELL_SIZE;
  ...
  if (assaultWorld) {
    _assaultWorldPadX = ws.padX;
    _assaultWorldPadY = ws.padY;
    _assaultWorldW    = W;
    _assaultWorldH    = H;
  } else {
    _assaultWorldPadX = 0;
  ...
  const spawnRef = options.spawn ?? { col: 0, row: Math.floor(ROWS / 2) };
  const goalRef = options.goal ?? { col: COLS - 1, row: Math.floor(ROWS / 2) };

  let skipProcedural = false;
  if (assaultWorld) {
    skipProcedural = bakeAssaultWorldTerrain(
      tc, COLS, ROWS, cs, W, H, ws.padX, ws.padY, spawnRef, goalRef, rng,
    );
    terrainUsesSprite = skipProcedural;
  } else {
    skipProcedural = options.ashfen === true
      && bakeAshfenTerrain(tc, COLS, ROWS, cs, W, H, rng, spawnRef, goalRef, {
        pathless: options.pathless === true,
      });
    if (skipProcedural) terrainUsesSprite = true;
  }

  // ── Base layer — procedural skirmish / fallback ──
  if (!skipProcedural) {
...
function clampGridPan() {
  const s = effectiveGridZoom();
  if (useAssaultScrollWorld()) {
    const clamped = clampAssaultGridPan(gridPanX, gridPanY, {
      worldWidth: _assaultWorldW,
      worldHeight: _assaultWorldH,
...
function spawnEnemy(type = ENEMY_TYPES.DRAUGR, hpScale = 1) {
  ...
  if (ENEMY_DEFS[type].flying) {
    const goalC = grid.cellCenter(GOAL.col, GOAL.row);
    if (useAssaultScrollWorld()) {
      const border = getAssaultSpawnPixel(_spawnCol, _spawnRow);
      path = [border, goalC];
    } else {
      path = [grid.cellCenter(_spawnCol, _spawnRow), goalC];
    }
  } else if (useAssaultScrollWorld()) {
    path = getAssaultEnemyPath(_spawnCol, _spawnRow);
  } else {
  ...
  scatterEnemySpawn(newEnemy, path);

  if (!_firstNightDramaShown && _campaignNodeMode && isFirstSagaMap(_campaignMapIndex) && _campaignNodeIndex === 0) {
    _firstNightDramaShown = true;
    _enemyIntroBanner = {
      label: 'FROM THE MIST',
      hint: 'Raiders stagger from the fen — hold the gate.',
      timer: 260,
      maxTimer: 260,
      category: 'undead',
    };
    ...
    portalFlash = 10;
    portalFlashColor = 'blue';
    combatHitStop = Math.max(combatHitStop, 8);
    _spawnMistDramaTimer = 90;
  } else if (!_assaultWave1SpawnDrama && _campaignNodeMode && isPathlessMode() && isFirstSagaMap(_campaignMapIndex) && waveNumber === 1) {
    _assaultWave1SpawnDrama = true;
    screenShake = Math.max(screenShake, 4);
    portalFlash = 8;
    portalFlashColor = 'blue';
    combatHitStop = Math.max(combatHitStop, 6);
    _spawnMistDramaTimer = 60;
  }

  return newEnemy;
}

/** Slight spawn scatter + lane offset so wave groups do not march in perfect lockstep. */
function scatterEnemySpawn(enemy, path) {
...
function spawnBoss(waveNum) {
  ...
  const path = useAssaultScrollWorld()
    ? getAssaultEnemyPath()
...
window.addEventListener('keydown', e => {
  ...
  if (gamePhase === 'nodeMap') {
    const progress = ensureCampaignProgress();
    if (e.key === 'ArrowLeft') {
      const prev = findAdjacentUnlockedRegion(progress, _campaignMapIndex, -1, isFirstSagaSliceLockedRegion);
      if (prev != null) switchCommandMapRegion(prev);
      return;
    }
    if (e.key === 'ArrowRight') {
      const next = findAdjacentUnlockedRegion(progress, _campaignMapIndex, 1, isFirstSagaSliceLockedRegion);
      if (next != null) switchCommandMapRegion(next);
      return;
    }
  }
  ...
  if (e.key === 'Escape') {
    ...
    if (_hallFocusDefenderId && gamePhase === 'betweenBattles'
        && shouldShowHallOfHeroesView(_warCampTab, _progressionBuilding)) {
      _hallFocusDefenderId = null;
      return;
    }
    if (_treasuryFocusKey && gamePhase === 'betweenBattles'
        && shouldShowFortressUpgradeView(_warCampTab, _progressionBuilding)) {
      _treasuryFocusKey = null;
      return;
    }
...
canvas.addEventListener('mousedown', e => {
  ...
  if (e.button === 0 && _battleMinimapRect && useAssaultScrollWorld() && gamePhase === 'playing' && !gameOver) {
    ...
    if (mouseX >= mm.x && mouseX <= mm.x + mm.w && mouseY >= mm.y && mouseY <= mm.y + mm.h) {
      ...
      const s = effectiveGridZoom();
      const localX = worldX - _assaultWorldPadX;
      const localY = worldY - _assaultWorldPadY;
      const cx = COLS * CELL_SIZE * 0.5;
      const cy = ROWS * CELL_SIZE * 0.5;
      gridPanX = playfieldWidth() / 2 - cx - (localX - cx) * s;
      gridPanY = playfieldHeight() / 2 - cy - (localY - cy) * s;
      clampGridPan();
      return;
    }
  }
    ...
    if (e.button === 0) {
      for (const btn of _nodeMapBtns) {
        if (mouseX >= btn.x && mouseX <= btn.x + btn.w &&
            mouseY >= btn.y && mouseY <= btn.y + btn.h) {
          if (btn.action === 'back') {
            if (_campaignRegionActive && _campaignMapIndex != null) enterSettlementHub();
            else gamePhase = 'campaignSelect';
          } else if (btn.action === 'returnToSettlement' || btn.action === 'warCamp' || btn.action === 'settlementHub') {
            enterSettlementHub();
          } else if (btn.action === 'switchRegion') {
            switchCommandMapRegion(btn.mapIndex);
          } else if (btn.action === 'openRegionPicker') {
            gamePhase = 'campaignSelect';
          } else if (btn.action === 'skirmish') {
            _hintSeen.skirmishDiscovery = true;
            _skirmishDiscoveryTimer = 0;
  ...
  if (gamePhase === 'settlementHub') {
    if (e.button === 0) {
      for (const btn of _hubBtns) {
        if (mouseX >= btn.x && mouseX <= btn.x + btn.w &&
            mouseY >= btn.y && mouseY <= btn.y + btn.h) {
          handleHubBuildingAction(btn.action);
          return;
        }
      }
    }
      ...
      for (let bi = _betweenBtns.length - 1; bi >= 0; bi--) {
        ...
        if (mouseX >= btn.x && mouseX <= btn.x + btn.w &&
          ...
          if (btn.action === 'fightAgain') {
                ...
                if (_bond.defenderIds.includes(_retDef.defenderId)) {
                  ...
                  if (_survivor && !_survivor.scars?.includes('bond_grief')) {
                    ...
                    _promotionQueue.push({
                      defenderName: _survivor.name,
                      rankLabel:    'BOND GRIEF',
                      text:         `${_survivor.name} mourns ${_retDef.name}. They carry Bond Grief.`,
                      type:         'scar',
                    });
                  }
                }
            ...
            _promoBannerTimer = 0;
            if (_promotionQueue.length === 0) _promotionQueueInitial = 0;
            ...
            const maxScroll = shouldShowHallOfHeroesView(_warCampTab, _progressionBuilding)
            ...
            if (!_rg.ok) {
              ...
              const _starGate = TOWER_STAR_GATES[_recruitType];
              if (_starGate != null && stars < _starGate) {
                _uiToast = { text: `Requires ${ _starGate } ★ to recruit`, timer: 140, color: UI_COLORS.gold };
              } else {
              const id   = _generateId();
          ...
          } else if (btn.action === 'returnToSettlement') {
            enterSettlementHub();
          } else if (btn.action === 'switchTab') {
            if (_progressionBuilding) return;
            _warCampTab = btn.tab;
...
function update() {
  ...
  if (_spawnMistDramaTimer > 0) _spawnMistDramaTimer--;
  ...
  for (let i = bullets.length - 1; i >= 0; i--) {
    ...
    if (b.lastHitDmg > 0 && b.target) {
      const isKill = reward > 0;
      const isCrit = b.lastHitDmg >= 28 || (b.damage >= 30 && b.lastHitDmg >= b.damage * 0.9);
      const feel = computeHitFeel(b.lastHitDmg, {
        isCrit,
        isKill,
        isBossHit: !!b.bossDmg,
        campaign: isCampaignAssaultBattle(),
      });
      screenShake = Math.max(screenShake, feel.shake);
      combatHitStop = Math.max(combatHitStop, feel.hitStop);
      applyEnemyHitKnock(b.target, b.lastHitDmg, b.x, b.y, { campaign: isCampaignAssaultBattle() });
    }
    // Boss hit (not kill): strong impact ring
    if (b.bossDmg) {
    ...
    if (reward > 0) {
      ...
      if (killBoss) {
        ...
        if (b.target && isPathlessMode() && isCampaignAssaultBattle()) {
          const flair = applyEnemyDeathFlair(b.target, _killType ?? 'raider');
          if (flair.shake) screenShake = Math.max(screenShake, flair.shake);
          for (const burst of flair.particles ?? []) {
            spawnParticles(burst.x, burst.y, burst.color, burst.count ?? 5);
          }
        }
  ...
  if (isPathlessMode() && SPAWN && GOAL) {
    tickPathlessFootDust();
    tickLaneWearMarks();
    const dustPhase = _frameTick * 0.5;
    const _campAssault = isCampaignAssaultBattle();
    for (const e of enemies) {
      if (!e.alive || e.reached) continue;
      if (!isOnPathlessLane(e.x, e.y, SPAWN, GOAL, CELL_SIZE)) continue;
      const spd = e.speed ?? 0;
      if (spd < 0.08) continue;
      if (Math.floor(dustPhase + e.x * 0.08) % 3 !== 0) continue;
      emitPathlessFootDust(e.x, e.y + e.radius * 0.45, Math.min(1.3, spd * 0.45), { campaign: _campAssault });
      if (Math.floor(dustPhase + e.x * 0.05) % 9 === 0) {
        emitLaneWearMark(e.x, e.y + e.radius * 0.55, Math.min(1.2, spd * 0.35));
      }
    }
  }
...
function drawThreatIntelCard() {
  ...
  const cardW   = assaultPlayfieldWide() ? 204 : 118;
  ...
  if (isBossW) {
    ...
    const threatLine = isCampaignCombat()
      ...
      : formatThreatStatusLine({ onField, incoming, waveTotal, comp: waveComposition(dispW) });
    ctx.fillStyle = 'rgba(210,195,160,0.92)';
    ctx.font = 'bold 7px monospace';
    const maxTextW = cardW - 14;
    let line = threatLine;
    while (line.length > 4 && ctx.measureText(line).width > maxTextW) {
      line = line.slice(0, -1);
    }
    if (line !== threatLine) line += '…';
    ctx.fillText(line, cardX + 8, cardY + 22);
...
function drawDefenderSidebarRow(x, y, w, rightX, tower, { isMvp = false, isFieldSelected = false, maxScore = 0 } = {}) {
  ...
  if (isCampaignAssaultBattle() && waveState === 'active') {
    ...
    drawPanelHpBar(ctx, x + 30, y + CARD_H - 7, w - 38, 4, hpFrac, combatFieldHpAccent(hpFrac));
  }
...
function drawRingGateMarkers() {
  if (!hasFortressRing()) return;
  // Campaign assault — palisade art + west gate sprite; four gold lanterns read as debug UI.
  if (_campaignNodeMode && isPathlessMode()) return;
  const cs = CELL_SIZE;
  const _R = FORTRESS_RING_R;
...
function drawFortressZoneRing() {
  ...
  // Subtle warm tint on empty cells inside the fortress zone — not green moss
  const cellPx = gridScreenCell(cs);
  ctx.fillStyle = 'rgba(120,90,55,0.07)';
  ...
  ctx.strokeStyle = `rgba(180,140,80,${_pulse + 0.08})`;
  ctx.lineWidth = 1.8;
  ctx.setLineDash([5, 4]);
  ctx.strokeRect(cx - half, cy - half, half * 2, half * 2);
  ctx.setLineDash([]);
  ctx.font = 'bold 8px monospace';
  ctx.fillStyle = `rgba(210,175,110,${_pulse + 0.22})`;
  ctx.textAlign = 'center';
...
function drawRightPanel() {
  ...
  const _glass   = assaultUiGlass();
  const _assaultTransparent = assaultPlayfieldWide() && !isCampaignAssaultBattle();
  ...
  } else if (_campaignHudOnly) {
    drawFantasyPanel(px, GRID_TOP, pw, fullH, 'rgba(10,6,20,0.94)', 0.78);
  }
  ...
  if (waveState === 'active') {
    ...
    // ── DEFENDER STATS (aggregate — roster on left panel) ────────────────────
    {
      ...
      _row('Damage:', formatBattleStat(totalDmg), UI_COLORS.gold);
...
function drawFieldPrepPanel(px, pw, fullH) {
  ...
  ctx.fillText('◀ SETTLEMENT', cx, ly + 17);
  _fieldPrepBtns.push({ x: btnX, y: ly, w: btnW, h: secH, action: 'settlementHub' });
  ly += secH + 6;
  drawFantasyPanel(btnX, ly, btnW, secH, 'rgba(12,8,4,0.85)', 0.6, 4);
  ...
  ctx.fillText('Assign posts on the left · grow your settlement between assaults', cx, ly);
}
...
function drawCampaignMetaBar(center) {
  ...
  if (gamePhase === 'nodeMap') {
    const progress = ensureCampaignProgress();
    const run = getMapRun(progress, _campaignMapIndex);
    const header = getCommandMapMetaHeader(progress, _campaignMapIndex, run, _campaignState);
    metaCenter = {
      line1: header.line1,
      line2: header.line2,
      color: UI_COLORS.gold,
      line2Color: header.line2 ? 'rgba(240,200,100,0.72)' : undefined,
    };
    subtitle = 'COMMAND MAP';
  } else if (gamePhase === 'fortressPrep') {
    const assault = _pendingAssaultNode != null
      ? getAssaultInfo(_campaignMapIndex, _pendingAssaultNode) : null;
    const prepHint = getPrepInstructionHint(getPrepShellPanelCtx());
    subtitle = prepHint?.line
      ?? (assault
    ...
    if (_progressionBuilding === 'warband' && isHallOfHeroesViewReady()) {
    ...
    } else if (_progressionBuilding === 'fortress' && isTreasuryViewReady()) {
      const treasuryHint = getTreasuryInstructionHint({
        focusKey: _treasuryFocusKey,
        goldReserve,
      });
      subtitle = treasuryHint.line;
      metaCenter = { line1: treasuryHint.title, line2: treasuryHint.line, color: UI_COLORS.gold };
    } else if (
      (_progressionBuilding === 'recruit' || (_warCampTab === 'recruit' && !_progressionBuilding))
      && isCampaignWarCamp()
    ) {
      const header = getBarracksMetaHeader();
      const gate = canRecruitInCampaignWarCamp();
      const chrome = getBarracksChromeGuidance({
        selectedType: _recruitType,
        goldReserve,
      ...
      metaCenter = {
        line1: header.line1,
        line2: chrome.subtitle || header.line2,
        color: UI_COLORS.gold,
      };
      ...
      const _fu = goldReserve > 0
        ...
        : null;
      const _wcHint = getWarCampInstructionHint({
        tabPulseTarget: isSimplifiedWarCamp(_campaignMapIndex ?? 0) ? _warCampTabPulse : null,
  ...
  if (gamePhase === 'betweenBattles' && isCampaignWarCamp() && !_progressionBuilding) {
    drawWarCampFrameCycle(ctx, BASE_W, FRAME_THICK, _warCampTab);
  }
...
function drawOnboardingBanner() {
  if (isFortressPrepPhase()) {
    ...
    if (prepHint) {
      ...
      ctx.strokeStyle = prepHint.urgent
        ? 'rgba(220,140,60,0.55)'
        : 'rgba(212,175,55,0.45)';
      ctx.lineWidth = 0.8;
      ctx.stroke();
      ctx.font = 'bold 7px monospace';
      ctx.fillStyle = prepHint.urgent ? '#f0a050' : UI_COLORS.gold;
      ctx.fillText(prepHint.title, BASE_W / 2, y + 2);
      ctx.font = '6.5px monospace';
      ctx.fillStyle = UI_COLORS.parchment;
      ctx.fillText(prepHint.line, BASE_W / 2, y + 12);
      ctx.restore();
      return;
    }
  }
...
function drawEquipCeremony() {
  ...
  if (def) {
    ...
    if (ringA > 0.02) {
      ...
      if (!drawEquipRingArt(ctx, BASE_W / 2, layout.ringCy, ringR, ringA)) {
...
function drawTopBar() {
  ...
  // Active wave — unified threat line (THREAT card handles assault wide layout)
  if (waveState === 'active' && !assaultPlayfieldWide()) {
    const onField  = enemies.filter(e => e.alive && !e.reached).length;
    const incoming = spawnQueue.length;
    const comp     = isCampaignCombat() ? null : waveComposition(waveNumber);
    const threatLine = isCampaignCombat()
...
/** Bottom-right minimap during scrollable campaign assault. */
function drawBattleMinimapOverlay() {
  if (!useAssaultScrollWorld() || gamePhase !== 'playing' || gameOver) {
    _battleMinimapRect = null;
    return;
  }
  ...
  drawBattleMinimap(ctx, {
    ...
    viewportWidth: playfieldWidth(),
    viewportHeight: playfieldHeight(),
    viewLeft: _assaultWorldPadX + viewGridLeft,
    viewTop: _assaultWorldPadY + viewGridTop,
    zoom: s,
    goal: GOAL,
    spawn: SPAWN,
    spawnWorldX: _assaultWorldPadX + borderSpawn.x,
    spawnWorldY: _assaultWorldPadY + borderSpawn.y,
    cellSize: CELL_SIZE,
    padX: _assaultWorldPadX,
    padY: _assaultWorldPadY,
    towers: towers.map(t => ({ x: t.x, y: t.y, defenderId: t.defenderId })),
...
function drawBossWarning() {
  ...
  const _nextCampaignBoss = _campaignNodeMode && _nodeWavePlan?.waves?.[_nodeWaveIndex + 1]?.isBoss
    ? getAssaultInfo(_campaignMapIndex, _campaignNodeIndex) : null;
  const bossLabel = bossCfg?.name ?? (_nextCampaignBoss?.codename === 'Ash-Warden' ? 'ASH-WARDEN' : 'BOSS');
  const isAshWardenBanner = bossLabel === 'ASH-WARDEN';
  const bannerY = GRID_TOP + 36;
  const _bossHasPhase = bossCfg?.phase50SlowImmune || bossCfg?.phase75 || bossCfg?.phase25;
  const bannerH = isAshWardenBanner ? 52 : (bossCfg?.hint ? (_bossHasPhase ? 56 : 46) : (_bossHasPhase ? 44 : 34));
  const bannerW = isAshWardenBanner ? 320 : 280;

  ctx.save();
  ctx.globalAlpha = bossWarnAlpha;
  if (!isAshWardenBanner || !drawBossBannerArt(ctx, cx, bannerY - 4, bannerW, bannerH, bossWarnAlpha)) {
...
function _buildChronicleOC(filteredBattles, hallHonored, hallFallen, bonds, uniqueDefs, CW) {
  ...
  for (const battle of filteredBattles) {
    ...
    if (battle.mvpId) {
      const mvpDef = _roster?.find(battle.mvpId);
      const mvpName = mvpDef?.name ?? 'MVP';
      xc.font = '7px monospace';
      xc.fillStyle = 'rgba(160,140,100,0.50)';
      const mvpX = CW - 4 - xc.measureText(`MVP: ${mvpName}`).width;
      xc.fillText(`MVP: ${mvpName} (most kills)`, mvpX, y + 10);
    }
  ...
  if (bonds.length > 0 && !_chronicleDefFilter) {
    ...
    for (const bond of bonds) {
      ...
      xc.font = 'italic bold 8px monospace'; xc.fillStyle = '#c0b080';
...
function drawChronicleOverlay() {
  ...
  if (_chronicleOCH > listH + 8 && _chronicleScrollY < _chronicleOCH - listH - 4) {
    ctx.font = '7px monospace';
...
function drawCampaignAssaultDebrief(W, H, isVictory, fadeT) {
  ...
  const safe = getDebriefScrollSafeArea(panX, slideY, panW, panH);
  const hx = safe.cx;
  ...
  const field = _campaignMapIndex != null
    ? (getMapRun(ensureCampaignProgress(), _campaignMapIndex)?.fieldState ?? null)
    : null;
  ...
  ctx.textAlign = 'center';
  ctx.save();
  clipDebriefParchment(ctx, safe);

  if (assault) {
    drawParchmentInk(ctx, formatDebriefAssaultHeader(assault), hx, hy, {
  ...
  ctx.strokeStyle = 'rgba(48,28,8,0.55)';
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(safe.left + 4, hy);
  ctx.lineTo(safe.right - 4, hy);
  ctx.stroke();
  hy += 12;

  const proseAlpha = getDebriefContentAlpha(_debriefTimer);
  const proseLines = wrapText(ctx, `"${prose}"`, safe.width - 4);
  ...
  const fortressRows = damage.lines;
  const bodyPlan = planDebriefBodyLayout(safe, hy, {
    proseLineCount: proseLines.length,
    hasMvp: Boolean(mvpTower),
    contextLineCount: contextLines.length,
    hasBossLoot,
    fortressRowCount: fortressRows.length > 0 ? Math.min(3, fortressRows.length) : 0,
    isVictory,
  });

  for (const line of proseLines.slice(0, bodyPlan.proseMax)) {
    if (!debriefLineFits(hy, 12, safe)) break;
  ...
  if (mvpTower && debriefLineFits(hy, 11, safe)) {
  ...
  if (hasBossLoot && debriefLineFits(hy, 11, safe)) {
  ...
  for (const line of contextLines.slice(0, bodyPlan.contextMax)) {
  ...
  if (bodyPlan.fortressMax > 0 && debriefLineFits(hy, 13, safe)) {
    ...
    for (const row of fortressRows.slice(0, bodyPlan.fortressMax)) {
      if (!debriefLineFits(hy, 11, safe)) break;
      drawParchmentInk(ctx, row.label, safe.left + 2, hy, {
        font: 'bold 8px monospace',
        fill: '#1a0e04',
        align: 'left',
      });
      const val = row.value.length > 24 ? `${row.value.slice(0, 22)}…` : row.value;
      drawParchmentInk(ctx, val, safe.right - 2, hy, {
        font: 'bold 8px monospace',
        fill: toneColor[row.tone] ?? '#2a1808',
        align: 'right',
      });
      hy += 11;
    }
  ...
  const showNext = isVictory && prepNode != null;
  const showRetry = !isVictory && _campaignNodeIndex != null;
  const showReplay = isVictory && _campaignNodeIndex != null;
  const btnCount = (showNext ? 1 : 0) + (showRetry ? 1 : 0) + (showReplay ? 1 : 0) + 2;
  const btnW = btnCount > 3 ? 108 : 132;
  const gap = btnCount > 3 ? 8 : 10;
  ...
  if (_canContinue) {
    ...
    if (showReplay) {
      drawRoute(routeIdx++, () => {
        drawFantasyPanel(btnX, btnY, btnW, btnH, 'rgba(32,36,16,0.97)', 0.75, 6);
        ctx.font = 'bold 8px monospace';
        ctx.textAlign = 'center';
        ctx.fillStyle = '#d0b050';
        ctx.fillText('REPLAY ASSAULT', btnX + btnW / 2, btnY + 18);
        _debriefBtns.push({
          x: btnX, y: btnY, w: btnW, h: btnH,
          action: 'replayAssault',
          nodeIndex: _campaignNodeIndex,
        });
      });
      btnX += btnW + gap;
    ...
    drawRoute(routeIdx++, () => {
      ...
      ctx.fillText('SETTLEMENT', btnX + btnW / 2, btnY + 18);
      _debriefBtns.push({ x: btnX, y: btnY, w: btnW, h: btnH, action: 'settlementHub' });
    });
    btnX += btnW + gap;
...
function drawDebrief() {
  ...
  // Dark backdrop — fade independently so assault parchment stays readable
  ctx.save();
  ...
  if (_returnToNodeMapAfterDebrief && _canContinue) {
    ...
    const showRetry = !isVictory && _campaignNodeIndex != null;
    const showReplay = isVictory && _campaignNodeIndex != null;
    const btnCount = (nextNode != null ? 1 : 0) + (showRetry ? 1 : 0) + (showReplay ? 1 : 0) + 1;
    const btnW = btnCount > 2 ? 120 : 148;
    const gap = 10;
    const totalW = btnCount * btnW + (btnCount - 1) * gap;
    let btnX = hx - totalW / 2;
    ...
    if (nextNode != null) {
      ...
      ctx.fillStyle = '#e08060';
      ctx.fillText('RETRY ASSAULT', btnX + btnW / 2, btnY + 18);
      _debriefBtns.push({ x: btnX, y: btnY, w: btnW, h: btnH, action: 'retryAssault', nodeIndex: _campaignNodeIndex });
      btnX += btnW + gap;
    } else if (showReplay) {
      drawFantasyPanel(btnX, btnY, btnW, btnH, 'rgba(32,36,16,0.97)', 0.75, 6);
      ctx.font = 'bold 9px monospace'; ctx.textAlign = 'center';
      ctx.fillStyle = '#d0b050';
      ctx.fillText('REPLAY ASSAULT', btnX + btnW / 2, btnY + 18);
      _debriefBtns.push({ x: btnX, y: btnY, w: btnW, h: btnH, action: 'replayAssault', nodeIndex: _campaignNodeIndex });
      btnX += btnW + gap;
    }
    ...
    const campLabel = isVictory && assaultInfo
      ? `SETTLEMENT (${assaultInfo.codename} ✓)`
      : 'SETTLEMENT';
...
function drawSettlementHubScreen() {
  _hubBtns = [];
  const W = BASE_W, H = BASE_H;
  ...
  const layout = {
    ...
    y: top + 8,
    w: W - (FRAME_THICK + 8) * 2,
    h: H - top - FRAME_THICK - 16,
  };
  const progress = ensureCampaignProgress();
  const nextNode = _pendingNextAssaultNode
    ?? getNextAvailableAssault(progress, _campaignMapIndex, null)?.nodeIndex;
  const nextAssault = nextNode != null ? getAssaultInfo(_campaignMapIndex, nextNode) : null;
  drawSettlementHub(ctx, layout, {
    campaignState: _campaignState,
    chronicleCount: _campaignState?.chronicle?.battles?.length ?? 0,
    battlesCompleted,
    simplifiedSaga: isSimplifiedWarCamp(_campaignMapIndex ?? 0),
...
/** Hall of Heroes — backdrop+statues (base) or dossier+roster (overlays, after frame). */
function drawBetweenBattlesHallImmersive(W, H, contentTop, contentBot, btSec, phase = 'base') {
  const useBottomSlot = Boolean(_fromSettlementHub || _progressionBuilding);
  const hallRect = computeHallImmersiveRect(FRAME_THICK, contentTop, contentBot, W, useBottomSlot);
  ...
  const guidance = getHallObjectiveGuidance({
    focusId: _hallFocusDefenderId,
    defenderCount: _roster.defenders.length,
  ...
  if (phase === 'base') {
    _hallImmersiveActive = true;
    ctx.globalAlpha = btSec(0.22);
    drawHallOfHeroesView(ctx, hallRect, { ...hallOpts, phase: 'base' });
    if (_roster.defenders.length === 0) {
      ctx.textAlign = 'center';
      ctx.font = '10px monospace';
      ctx.fillStyle = 'rgba(120,100,70,0.45)';
      ctx.fillText('No defenders in warband yet', hallRect.x + hallRect.w / 2, hallRect.y + hallRect.h / 2);
      ctx.textAlign = 'left';
    }
    ctx.globalAlpha = 1;
    return;
  ...
  if (phase !== 'overlays') return;

  const { layout } = drawHallOfHeroesView(ctx, hallRect, { ...hallOpts, phase: 'overlays' });

  drawHallImmersiveChrome(ctx, hallRect, layout, {
    guidance,
    showBackToTown: useBottomSlot,
  }, _betweenBtns);
  ...
  if (_renameState) {
    ...
    ctx.fillText('Press Enter to confirm · Esc to cancel', W / 2, contentBot - 4);
    ctx.restore();
  }

  if (!useBottomSlot) {
    drawWarCampBottomBar(ctx, { baseW: W, baseH: H, frameThick: FRAME_THICK }, {
      tabPulseTarget: isSimplifiedWarCamp(_campaignMapIndex) ? _warCampTabPulse : null,
      activeTab: _warCampTab,
      nextAssault: _nai ? {
        codename: _nai.codename,
        tierLabel: _nai.tierLabel,
        nodeIndex: _prepNode,
        isRetry: _battleResult === 'defeat' && _nextNode == null,
      } : null,
      fortressUpgrade: _fu,
      showReturnToHub: false,
      buildingOnly: false,
    }, _betweenBtns);
  }
}
...
function drawBetweenBattlesTreasuryImmersive(W, H, contentTop, contentBot, btSec, phase = 'base') {
  ...
  const upgrades = _campaignState?.fortressUpgrades ?? {};
  const _fu = goldReserve > 0
    ? getNextFortressUpgradeOffer(upgrades, goldReserve)
    : null;
  ...
  const guidance = getTreasuryObjectiveGuidance({
    focusKey: _treasuryFocusKey,
    goldReserve,
    nextUpgrade: _fu,
  });
  ...
  if (phase === 'base') {
    _treasuryImmersiveActive = true;
    ctx.globalAlpha = btSec(0.22);
    drawTreasuryView(ctx, treasuryRect, { ...treasuryOpts, phase: 'base' });
    ctx.globalAlpha = 1;
    return;
  }
  ...
  const { layout } = drawTreasuryView(ctx, treasuryRect, { ...treasuryOpts, phase: 'overlays' });

  drawHallImmersiveChrome(ctx, treasuryRect, layout, {
    guidance,
    showBackToTown: useBottomSlot,
  }, _betweenBtns);
...
/** Barracks — backdrop+statues (base) or panels+chrome (overlays, after frame). */
function drawBetweenBattlesBarracksImmersive(W, H, contentTop, contentBot, btSec, phase = 'base') {
  const useBottomSlot = Boolean(_fromSettlementHub || _progressionBuilding);
  const barracksRect = computeBarracksImmersiveRect(FRAME_THICK, contentTop, contentBot, W, useBottomSlot);

  const upgrades = _campaignState?.fortressUpgrades ?? {};
  const barracksLevel = upgrades.barracks ?? 0;
  const _recruitGate = canRecruitInCampaignWarCamp();
  const recruitTypes = getRecruitableTypes({
    firstSagaMap: isCampaignWarCamp() && isFirstSagaMap(_campaignMapIndex),
  });
  ...
  const guidance = getBarracksChromeGuidance({
    selectedType: _recruitType,
    goldReserve,
    recruitCost: _effectiveRecruitCost,
    recruitAllowed: _recruitGate.ok,
  ...
  if (phase === 'base') {
    _barracksImmersiveActive = true;
    ctx.globalAlpha = btSec(0.22);
    drawBarracksView(ctx, barracksRect, { ...barracksOpts, phase: 'base' });
    ctx.globalAlpha = 1;
    return;
  }
  ...
  const { layout } = drawBarracksView(ctx, barracksRect, { ...barracksOpts, phase: 'overlays' });

  drawHallImmersiveChrome(ctx, barracksRect, layout, {
    guidance,
    showBackToTown: useBottomSlot,
  }, _betweenBtns);
...
function drawBetweenBattles() {
  _betweenBtns = [];
  _hallImmersiveActive = false;
  _treasuryImmersiveActive = false;
  _barracksImmersiveActive = false;
  const W = BASE_W, H = BASE_H;
  ...
  const _btFadeLeft = _betweenFadeIn;
  if (_betweenFadeIn > 0) _betweenFadeIn--;
  const btSec = (delay) => (_btFadeLeft <= 0 ? 1 : fadeAlpha * computeBetweenSectionAlpha(_btFadeLeft, delay));
  ctx.save();
  ctx.globalAlpha = fadeAlpha;
  ...
  const _preflightHall = isCampaignWarCamp()
    && shouldShowHallOfHeroesView(_warCampTab, _progressionBuilding);
  const _preflightTreasury = isCampaignWarCamp()
    && shouldShowFortressUpgradeView(_warCampTab, _progressionBuilding);
  const _preflightBarracks = isCampaignWarCamp()
    && shouldShowBarracksRecruitView(_warCampTab, _progressionBuilding);
  if (isCampaignWarCamp() && !_preflightHall && !_preflightTreasury && !_preflightBarracks) {
  ...
  const _barracksView = isCampaignWarCamp()
    && shouldShowBarracksRecruitView(_warCampTab, _progressionBuilding);
  const _immersiveView = _hallView || _treasuryView || _barracksView;
  const _contentTop = _wcLayout
    ? (_immersiveView && _progressionBuilding
      ? FRAME_THICK + META_TOP_BAR_COMPACT_H + 4
      : getWarCampContentTop(FRAME_THICK))
    : META_SCREEN_TOP;
  const _contentBot = H - FRAME_THICK - 8 - (_wcLayout && !_immersiveView ? WAR_CAMP_BOTTOM_BAR_H : 0);

  if (_hallView) {
    drawBetweenBattlesHallImmersive(W, H, _contentTop, _contentBot, btSec, 'base');
    ctx.restore();
    return;
  }

  if (_treasuryView) {
    drawBetweenBattlesTreasuryImmersive(W, H, _contentTop, _contentBot, btSec, 'base');
    ctx.restore();
    return;
  }

  if (_barracksView) {
    drawBetweenBattlesBarracksImmersive(W, H, _contentTop, _contentBot, btSec, 'base');
    ctx.restore();
    return;
  }
  ...
  if (isCampaignWarCamp()) {
    ...
    drawCampaignWarCampBriefing(ctx, { x: lpX, y: lpY, w: lpW, h: lpH }, {
      ...
      nextAssault: _nai ? {
        codename: _nai.codename,
        tierLabel: _nai.tierLabel,
        frontLabel: FRONT_LABELS[_nai.frontId] ?? 'West',
        waveCount: _nai.waveCount,
        nodeIndex: _prepNode,
        isRetry: _battleResult === 'defeat' && _nextNode == null,
      } : null,
      goldReserve,
    ...
    if (_chronicleUnread) {
      const _cb = _betweenBtns.find(b => b.action === 'openChronicle');
      if (_cb) {
        const _dotPulse = 0.75 + 0.25 * Math.sin(performance.now() / 300);
        ctx.fillStyle = `rgba(240,180,60,${_dotPulse.toFixed(2)})`;
        ctx.beginPath();
        ctx.arc(_cb.x + _cb.w - 6, _cb.y + 6, 4, 0, Math.PI * 2);
        ctx.fill();
      }
    }
  } else {
  ...
  // Buttons at bottom of left panel (skirmish only)
  ...
  _navBtns = [];
  const _fortUpgradePulse = !_progressionBuilding && _warCampTab === 'warband'
    ? getNextFortressUpgradeOffer(_campaignState?.fortressUpgrades ?? {}, goldReserve)
    : null;
  const _tabPulse = _progressionBuilding ? null : (_warCampTabPulse ?? (_fortUpgradePulse ? 'fortress' : null));
  let _bbNavH = 0;
  if (_progressionBuilding) {
    ctx.textAlign = 'center';
    ctx.font = 'bold 10px monospace';
    ctx.fillStyle = UI_COLORS.gold;
    ctx.fillText(getProgressionBuildingTitle(_progressionBuilding), rpX + rpW / 2, rpY + 14);
    ctx.font = '6px monospace';
    ctx.fillStyle = 'rgba(160,140,100,0.55)';
    ctx.fillText('Kingdom growth — return to town when done', rpX + rpW / 2, rpY + 26);
    ctx.textAlign = 'left';
    _bbNavH = 32;
  } else {
    _bbNavH = drawHorizTabs(rpX, rpY, rpW, WAR_CAMP_TABS, _warCampTab, _navBtns, { pulseId: _tabPulse });
  }
  if (_wcLayout && _warCampWelcomeTimer > 0 && !_progressionBuilding) {
  ...
  const _wcSimple = isCampaignWarCamp() && isSimplifiedWarCamp(_campaignMapIndex);
  const _wcTabHint = {
    warband:  _equipmentInventory.length > 0
      ? `Tap gold ⚔ / 🛡 slots on each card to equip (${_equipmentInventory.length} in stash)`
      : (_wcSimple ? 'Tap ⚔ / 🛡 on a card to manage gear · 📜 for bio' : 'Tap ⚔ / 🛡 slots to cycle equipment · 📜 bio'),
    recruit:  `Hire defenders · ${_effectiveRecruitCost}g each`,
    fortress: goldReserve > 0
  ...
  if (_wcLayout && !_progressionBuilding && !_hallView) {
    const _wcGuidance = getWarCampTabGuidance({
      activeTab: _warCampTab,
      tabPulseTarget: isSimplifiedWarCamp(_campaignMapIndex) ? _warCampTabPulse : null,
      nextAssault: _pendingNextAssaultNode != null || _battleResult === 'defeat'
        ? (() => {
            const _wcProgress = ensureCampaignProgress();
            const _nextNode = _pendingNextAssaultNode ?? getNextAvailableAssault(_wcProgress, _campaignMapIndex, null)?.nodeIndex;
            const _prepNode = _nextNode ?? (_battleResult === 'defeat' ? _campaignNodeIndex : null);
            const _nai = _prepNode != null ? getAssaultInfo(_campaignMapIndex, _prepNode) : null;
            return _nai ? { codename: _nai.codename } : null;
          })()
        : null,
      equipmentCount: _equipmentInventory.length,
      fortressUpgrade: goldReserve > 0
        ? getNextFortressUpgradeOffer(_campaignState?.fortressUpgrades ?? {}, goldReserve)
        : null,
      goldReserve,
    });
    _wcCursor += drawWarCampGuidanceCard(ctx, rix, _wcCursor, riW, _wcGuidance, _betweenBtns);
  }
  if (_wcLayout && _warCampTab !== 'warband' && !_progressionBuilding) {
    drawWarCampSectionBanner(ctx, rix, _wcCursor, riW, WAR_CAMP_BANNER_H, _warCampTab);
    _wcCursor += WAR_CAMP_BANNER_H + 6;
  }
  if (_wcLayout) {
  ...
  if (_warCampTab === 'warband' && _progressionBuilding !== 'runeSmith') {
    ...
    ctx.fillText(
      `${rCount} hero${rCount !== 1 ? 'es' : ''}  ·  Tank ${_wb.counts.tank}  DPS ${_wb.counts.st_dps + _wb.counts.aoe_dps}`,
      rix,
      _wcCursor + 8,
    );
    _wcCursor += 14;
    if (!_wcSimple && _equipmentInventory.length > 0) {
      ctx.font = 'bold 7px monospace';
      ctx.fillStyle = 'rgba(200,170,80,0.75)';
      ctx.fillText(`◈ ${_equipmentInventory.length} item${_equipmentInventory.length !== 1 ? 's' : ''} in stash — tap ⚔ or 🛡 on a hero card`, rix, _wcCursor + 8);
      _wcCursor += 12;
    }
    if (!_wcSimple && goldReserve > 0) {
      const _fuHint = getNextFortressUpgradeOffer(_campaignState?.fortressUpgrades ?? {}, goldReserve);
      ctx.font = 'bold 7px monospace';
      ctx.fillStyle = _fuHint ? 'rgba(120,200,120,0.8)' : 'rgba(160,140,100,0.65)';
      ctx.fillText(_fuHint
        ? `◆ ${goldReserve}g — FORTRESS tab · upgrade ${_fuHint.label} (${_fuHint.cost}g)`
        : `◆ ${goldReserve}g in reserve — open FORTRESS tab`, rix, _wcCursor + 8);
      _wcCursor += 12;
    }
    if (!_wcSimple) {
  ...
  // Rune Carver — Rune Smith building or warband (post-saga); hidden in First Saga slice
  const _showRunePanel = _progressionBuilding === 'runeSmith'
    || (_warCampTab === 'warband' && stars > 0 && !_wcSimple && !_progressionBuilding);
  if (_showRunePanel) {
    if (_progressionBuilding === 'runeSmith') _runeCarverExpanded = true;
    if (!_runeCarverExpanded && _progressionBuilding !== 'runeSmith') {
  ...
  if (_warCampTab === 'warband' && _promotionQueue.length > 0) {
    ...
    if (_promotionQueue.length > 0) {
      ...
      const pbCol  = prom.type === 'scar'      ? 'rgba(200,80,30,0.18)'   :
                     ...
                     prom.type === 'title'     ? 'rgba(100,100,140,0.18)' :
      ...
      const pbBdr  = prom.type === 'scar'      ? 'rgba(200,100,50,0.55)'  :
                     ...
                     prom.type === 'title'     ? 'rgba(160,160,200,0.65)' :
      ...
      const _accentColor = prom.type === 'scar'      ? 'rgba(200,100,60,0.8)'  :
                           ...
                           prom.type === 'title'     ? 'rgba(160,160,220,0.85)' :
      ...
      ctx.fillStyle = prom.type === 'scar'      ? '#e87050'  :
                      ...
                      prom.type === 'title'     ? '#b0b8e8'  :
                      prom.type === 'milestone' ? '#b8a870'  : '#88dd80';
      ctx.textAlign = 'left';
      const _promIcon = prom.type === 'title' ? '★' : prom.type === 'milestone' ? '◆' : '⬆';
      ...
      if (_promotionQueueInitial > 1) {
        const _promIdx = _promotionQueueInitial - _promotionQueue.length + 1;
        ctx.font = '7px monospace'; ctx.fillStyle = 'rgba(160,140,100,0.55)';
        ctx.textAlign = 'right';
        ctx.fillText(`${_promIdx} / ${_promotionQueueInitial}`, ackX - 3, ackY + 10);
        ctx.textAlign = 'left';
      }
  ...
  const listBot  = rpY0 + rpH0 - 6 - (_wcLayout ? WAR_CAMP_BOTTOM_BAR_H : 0);
  ...
  if (_wcLayout && _hallView) {
    ...
    _rosterScrollOffset = Math.max(0, Math.min(_rosterScrollOffset, maxGridRow));
    const startIdx = _rosterScrollOffset * grid.cols;
    ...
    if (_rosterScrollOffset < maxGridRow) {
    ...
    for (let vi = 0; vi < grid.cardsPerPage; vi++) {
      const defIdx = startIdx + vi;
      if (defIdx >= totalDefs) break;
      const def = _roster.defenders[defIdx];
      const { x: cx, y: cy } = warCampCardOrigin(rix, listTop, grid, vi);
      ...
      const slotMeta = [0, 1].map(slotIdx => {
        const itemId = def.equipment[slotIdx];
        const itemDef = itemId ? ITEM_DEFS[itemId] : null;
        return {
          itemDef,
      ...
      const chrome = drawWarCampPortraitCard(ctx, cx, cy, grid.cardW, grid.cardH, def, {
        selected: isRenaming,
        bond: !!_cardBond,
        isRenaming,
      ...
      ctx.fillStyle = 'rgba(30,20,40,0.8)'; ctx.strokeStyle = 'rgba(120,90,180,0.4)'; ctx.lineWidth = 0.6;
      ctx.beginPath(); ctx.roundRect(chrome.bioX, chrome.bioY, chrome.bioW, chrome.bioH, 2); ctx.fill(); ctx.stroke();
      ctx.font = '6px monospace'; ctx.fillStyle = 'rgba(160,130,200,0.75)'; ctx.textAlign = 'center';
      ctx.fillText('📜', chrome.bioX + chrome.bioW / 2, chrome.bioY + 9);
      ctx.textAlign = 'left';
      _betweenBtns.push({ x: chrome.bioX, y: chrome.bioY, w: chrome.bioW, h: chrome.bioH, action: 'openBio', defenderId: def.defenderId });
      ...
      if (!hasName || def.careerLevel >= 1) {
        ...
        ctx.beginPath(); ctx.roundRect(chrome.rnX, chrome.rnY, chrome.bioW, chrome.bioH, 2); ctx.fill(); ctx.stroke();
        ctx.font = '6px monospace'; ctx.fillStyle = isRenaming ? '#ffd040' : 'rgba(160,140,60,0.55)';
        ctx.textAlign = 'center';
        ctx.fillText(!hasName ? '✎' : '✏', chrome.rnX + chrome.bioW / 2, chrome.rnY + 9);
        ctx.textAlign = 'left';
        _betweenBtns.push({ x: chrome.rnX, y: chrome.rnY, w: chrome.bioW, h: chrome.bioH, action: 'startRename', defenderId: def.defenderId });
      }
    }
    ...
    if (totalDefs === 0) {
      ...
      ctx.textAlign = 'left';
    } else if (totalDefs > grid.cardsPerPage) {
      const endIdx = Math.min(startIdx + grid.cardsPerPage, totalDefs);
      ctx.font = '8px monospace'; ctx.fillStyle = 'rgba(140,120,70,0.5)';
      ctx.textAlign = 'center';
      ctx.fillText(`${startIdx + 1}–${endIdx} of ${totalDefs}`, rpX + rpW / 2, listBot - 4);
      ctx.textAlign = 'left';
    }
  ...
  _roster.defenders.slice(_rosterScrollOffset, _rosterScrollOffset + maxRows).forEach((def, i) => {
    ...
    if (isPendingDismiss && _defBond) {
      ...
      ctx.fillText(`⚠ ${_bonded?.name ?? '?'} will carry Bond Grief`, rix, ry + 36);
      ...
      let _coLine = null;
      for (const [key, count] of Object.entries(_campaignState?.coDeployments ?? {})) {
        if (count < 3 || count >= 5) continue;
        const [idA, idB] = key.split(':');
        if (def.defenderId !== idA && def.defenderId !== idB) continue;
        const _alreadyBonded = (_campaignState?.bonds ?? []).some(b =>
          b.defenderIds.includes(idA) && b.defenderIds.includes(idB));
        if (_alreadyBonded) continue;
        const _partnerId = def.defenderId === idA ? idB : idA;
        const _partner = _roster?.find(_partnerId);
        _coLine = `With ${_partner?.name ?? '?'}: ${count}/5`;
        break;
      }
      if (_coLine) {
        ctx.font = '7px monospace'; ctx.fillStyle = 'rgba(140,130,100,0.45)';
        ctx.fillText(_coLine, rix, ry + 36);
  ...
  if (_warCampTab === 'recruit' && (!_progressionBuilding || _progressionBuilding === 'recruit')) {
    const _recruitGate = canRecruitInCampaignWarCamp();
    if (!_recruitGate.ok) {
      const _rcX = rix + riW / 2;
      // Dim locked panel
      ctx.fillStyle = 'rgba(8,6,4,0.60)';
      ctx.strokeStyle = 'rgba(80,65,45,0.28)'; ctx.lineWidth = 0.7;
      ctx.beginPath(); ctx.roundRect(rix, recY + 6, riW, 72, 4); ctx.fill(); ctx.stroke();
      // Shield icon
      ctx.font = '16px monospace'; ctx.fillStyle = 'rgba(110,95,65,0.40)';
      ctx.textAlign = 'center';
      ctx.fillText('⚔', _rcX, recY + 30);
      // Reason
      ctx.font = 'bold 9px monospace'; ctx.fillStyle = 'rgba(175,145,95,0.82)';
      ctx.fillText(_recruitGate.reason, _rcX, recY + 50);
      // Hint / preview
      ctx.font = '7px monospace'; ctx.fillStyle = 'rgba(130,110,75,0.55)';
      if (_recruitGate.reason.includes('Settlement')) {
        ctx.fillText('Complete the Settlement on the command map.', _rcX, recY + 63);
        ctx.fillStyle = 'rgba(110,95,65,0.38)';
        ctx.fillText('— Valkyrie · Archer will join —', _rcX, recY + 74);
      } else {
        ctx.fillText('Your warband is at full strength for this chapter.', _rcX, recY + 63);
      }
      ctx.textAlign = 'left';
    } else {
    // ── RECRUIT (full panel) ───────────────────────────────────────────────────
  ...
  } else if (_warCampTab === 'fortress' && (!_progressionBuilding || _progressionBuilding === 'fortress')) {
    // ── FORTRESS NODES — static art banner + upgrade list ─────
    const upgrades = _campaignState?.fortressUpgrades ?? {};
    ...
    ctx.font = 'bold 8px monospace';
    ctx.fillStyle = _nextFu ? 'rgba(140,200,120,0.85)' : 'rgba(140,120,90,0.58)';
    ctx.fillText(_nextFu
      ? `Tap UPGRADE to spend reserve gold · next: ${_nextFu.label} (${_nextFu.cost}g)`
      : 'FORTRESS BUILDINGS', rix, _fortStatusY + 10);
    const _nodeBaseY = _fortStatusY + 20;
    {
      ...
      if (_maxLvls > 0) {
        ...
        ctx.fillText(`${_totalLvls} / ${_maxLvls} levels`, rix + riW, _fortStatusY + 10);
        ctx.textAlign = 'left';
      }
    }
    const nodeRowH = 48;
  ...
  if (_renameState) {
    ...
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.roundRect(rpX - 2, rpY - 2, rpW + 4, rpH + 4, 8);
    ctx.stroke();
    ctx.font = '7px monospace';
    ctx.textAlign = 'center';
    ctx.fillStyle = 'rgba(255,200,80,0.72)';
    ctx.fillText('Press Enter to confirm · Esc to cancel', W / 2, _contentBot - 4);
    ctx.restore();
  } else if (isCampaignWarCamp() && performance.now() < _renameHintUntil) {
    ctx.save();
    ctx.globalAlpha = 0.65 + Math.sin(performance.now() * 0.005) * 0.2;
    ctx.font = '7px monospace';
    ctx.textAlign = 'center';
    ctx.fillStyle = 'rgba(200,170,100,0.75)';
    ctx.fillText('Tap ✏ on a defender to rename them', W / 2, _contentBot - 6);
    ctx.restore();
  }
...
function drawNodeMap() {
  ...
  const _showRegionNav = progress.mapsUnlocked > 1;
  const _regionNavH = _showRegionNav ? 34 : 0;
  const mapX = 40, mapY = _top + 8 + _regionNavH;
  const mapW = W - 80, mapH = H - 168 - (mapY - _top) - (_campaignRegionActive ? 8 : 0);
  drawFantasyPanel(mapX, mapY, mapW, mapH, 'rgba(8,12,8,0.95)', 0.65, 10);

  const briefingW = Math.min(268, Math.max(188, Math.floor(mapW * 0.38)));
  const briefingLines = getCommandMapBriefingLines(meta, {
    nodeCount,
    portalCount,
    deployHint: _deployHint,
    structRec: _structRec,
    wbAnalysis: _wbAnalysis,
    warnings: _warnings,
    tutorialNext: isTutorialNode(_campaignMapIndex, _nextNode),
  });
  drawObjectiveGuidanceChip(ctx, mapX + 10, mapY + 10, briefingW, 'WHAT TO DO NOW', briefingLines);
  ...
  if (_showRegionNav) {
    const navY = mapY - _regionNavH + 6;
    const prevIdx = findAdjacentUnlockedRegion(progress, _campaignMapIndex, -1, isFirstSagaSliceLockedRegion);
    const nextIdx = findAdjacentUnlockedRegion(progress, _campaignMapIndex, 1, isFirstSagaSliceLockedRegion);
    const chipH = 30;
    const chipW = 108;

    if (prevIdx != null) {
      const prevMeta = getCampaignMapMeta(prevIdx);
      const prevShort = (prevMeta?.name ?? 'Prev').split(' ')[0];
      drawWarCampGlassChip(ctx, mapX, navY, chipW, chipH, {
        title: '◀ REGION',
        subtitle: prevShort,
        action: 'switchRegion',
        extra: { mapIndex: prevIdx },
        btnsOut: _nodeMapBtns,
      });
    }
    if (nextIdx != null) {
      const nextMeta = getCampaignMapMeta(nextIdx);
      const nextShort = (nextMeta?.name ?? 'Next').split(' ')[0];
      drawWarCampGlassChip(ctx, mapX + mapW - chipW, navY, chipW, chipH, {
        title: 'REGION ▶',
        subtitle: nextShort,
        action: 'switchRegion',
        extra: { mapIndex: nextIdx },
        btnsOut: _nodeMapBtns,
      });
    }
    if (progress.mapsUnlocked > 2) {
      const allW = 96;
      drawWarCampGlassChip(ctx, W / 2 - allW / 2, navY + chipH + 4, allW, 22, {
        title: 'ALL REGIONS',
        subtitle: `${progress.mapsUnlocked} unlocked`,
  ...
  if (_campaignRegionActive) {
    const chipPad = 10;
    const bw = 124;
    const bh = 34;
    const bx = W - FRAME_THICK - bw - chipPad;
    const by = H - FRAME_THICK - bh - chipPad;
...
function _drawFrontAssaultPanel(progress, layout, run, mapX, mapY, mapW, mapH, frontId) {
  ...
  for (const assault of front.assaults) {
    ...
    if (!assault.isBoss) {
      ctx.font = '7px monospace';
      ctx.fillStyle = clearedRow ? 'rgba(100,150,80,0.65)' : 'rgba(140,120,80,0.55)';
      ctx.fillText(clearedRow ? 'Replay for gold' : assault.codename, listX, rowY + 12);
    }
...
function draw() {
  ...
  if (gamePhase === 'betweenBattles') {
    ...
    drawWarCampWelcome();
    drawFrames();
    if (_hallImmersiveActive) {
      const { contentTop: _hallContentTop, contentBot: _hallContentBot } = getImmersiveWarCampBounds(BASE_W, BASE_H);
      drawBetweenBattlesHallImmersive(BASE_W, BASE_H, _hallContentTop, _hallContentBot, () => 1, 'overlays');
    }
    if (_treasuryImmersiveActive) {
      const { contentTop: _treasuryContentTop, contentBot: _treasuryContentBot } = getImmersiveWarCampBounds(BASE_W, BASE_H);
      drawBetweenBattlesTreasuryImmersive(BASE_W, BASE_H, _treasuryContentTop, _treasuryContentBot, () => 1, 'overlays');
    }
    if (_barracksImmersiveActive) {
      const { contentTop: _barracksContentTop, contentBot: _barracksContentBot } = getImmersiveWarCampBounds(BASE_W, BASE_H);
      drawBetweenBattlesBarracksImmersive(BASE_W, BASE_H, _barracksContentTop, _barracksContentBot, () => 1, 'overlays');
    }
    drawCampaignMetaBar();
  ...
  setPlayfieldSpriteCompensation(
    useAssaultScrollWorld() ? ASSAULT_UNIT_SCALE
      : assaultPlayfieldWide() ? 0.9 / Math.max(1, _pfScale) : 1,
  );
  ...
  if (assaultPlayfieldWide() && _campaignNodeMode) {
    drawCampaignAssaultPlayfieldBackdrop(ctx, _pfLeft, GRID_TOP, _pfW, _pfH, time, {
      scrollAssault: useAssaultScrollWorld(),
    });
  }
  if (screenShake > 0) screenShake *= 0.86;
  const _shakeCap = isCampaignAssaultBattle() ? 8 : 4;
  const _shakeMag = screenShake > 20 ? Math.min(screenShake, 10) : Math.min(screenShake, _shakeCap);
  ...
  grid.wallData    = wallData;
  grid.minimalGoalDecor = _campaignNodeMode && isPathlessMode();
  grid.useCampaignPalisade = _campaignNodeMode && isPathlessMode();
  grid.useAshfenSpawn = grid.useCampaignPalisade;
  grid.useAssaultFortressPlate = false;
  grid.usePalisadeRingArt = false;
  grid.campaignPrepMeta = _prepFieldMeta ?? null;
  ...
  if (!hideAssaultBattleGrid()) drawPath();
  if (isPathlessMode() && SPAWN && GOAL && !hideAssaultBattleGrid()) {
    drawPathlessAssaultLane(ctx, SPAWN, GOAL, COLS, ROWS, CELL_SIZE, time);
    drawLaneWearMarks(ctx);
  }
  if (_campaignNodeMode && isPathlessMode() && hideAssaultBattleGrid()) {
    drawCampaignAssaultColorGradeBg(ctx, COLS, ROWS, CELL_SIZE, GOAL, FORTRESS_RING_R, time);
  }
  drawExtraPortalPaths();
  drawRingGateMarkers();

  // ── Fortress warm light — large amber glow from fortress area ────────────
  if (!grid.minimalGoalDecor) {
  ...
  if (grid.useAshfenSpawn) {
    ...
    drawAnimatedSpawnFenMist(ctx, SPAWN, CELL_SIZE, time, {
      ...
      py: useAssaultScrollWorld() ? borderSpawn.y : undefined,
    });
  }
  ...
  for (const t of towers) {
    ...
    const campaignAssault = isCampaignAssaultBattle();
    const strength = hero && inCombat
      ? (campaignAssault ? 0.26 : 0.52)
      : (campaignAssault ? 0.62 : 1);
    drawDefenderPortraitGlow(t.x, t.y + 2, t.glowRgb, poolR, strength);
    if (hero && inCombat && t.glowRgb && !isCampaignAssaultBattle()) {
  ...
  drawFortificationsAndTowersDepthSorted(ctx, time);

  if (grid.useCampaignPalisade && hideAssaultBattleGrid()) {
    drawCampaignPalisadeTorchPools(ctx, wallData, CELL_SIZE, time, { strength: isCampaignAssaultBattle() ? 0.42 : 1 });
  }
  ...
  // Defender nameplates — deploy prep / selected only (name + damage text hid sprites in combat)
  if (canModifyWarbandDeployment() || towers.some(t => t.selected)) {
    ctx.save();
    ctx.textAlign    = 'center';
    ...
    for (const t of towers) {
      if (!t.name) continue;
      if (!canModifyWarbandDeployment() && !t.selected) continue;
      const fp   = t.footprint ?? { w: 1, h: 1 };
      const nx   = t.x;
  ...
  redrawHeroSpritesOverFortifications(ctx, time);
  if (isPathlessMode() && SPAWN && GOAL) {
    drawPathlessFootDust(ctx);
  }
